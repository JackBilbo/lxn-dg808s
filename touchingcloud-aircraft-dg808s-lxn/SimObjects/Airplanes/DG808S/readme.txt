...
Automatic installer contain most of required files, so only thing you need to do is select Community path. Exceptions are sound packages - we can't include any sounds into installer as it will be Microsoft policy violation,  so they will be copied by installer from default MSFS aircraft automatically.
    ...
    Check "Remove legacy files" option if you have DG808S already imported or installed
    Check "Aircraft files" if you are installing this update for the first time
    Check cockpit gauges if you want to update gauge files only
    Press Next button
...
If you have problems with automatic installer, you can unpack files manually, them perform these actions manually:
